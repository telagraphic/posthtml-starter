...
//   isMultiple: true
// },
root:{type:"string",alias:"r",default:"./"},allInOutput:{type:"boolean",default:!1,alias:"a"}}}),read=file=>new Promise(resolve=>{_fs.default.readFile(file,"utf8",(error,data)=>{error&&console.warn(error),resolve(data)})}),interopRequire=object=>object&&object.__esModule?object.default:object,getPlugins=config=>Object.keys(config.plugins||{}).map(plugin=>interopRequire(require(plugin))(config.plugins[plugin])),config=(0,_cfgResolve.default)(cli),processing=async file=>{const output=await(0,_outResolve.default)(file,config),plugins=Array.isArray(config.plugins)?config.plugins:getPlugins(config);(0,_makeDir.default)(_path.default.dirname(output)).then(read.bind(void 0,file)).then(html=>Promise.resolve((0,_posthtml.default)(plugins).process(html,{...config.options,from:file}))).then(({html})=>{_fs.default.writeFile(output,html,error=>{error&&console.warn(error),console.log(`The file ${file} has been saved!`)})})};_fastGlob.default.stream(config.input).on("data",processing).once("error",console.warn);